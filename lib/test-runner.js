  ...
  if (global.jmeterTransactions && Object.keys(global.jmeterTransactions).length > 0) {
    ...
    Object.entries(global.jmeterTransactions).forEach(([userId, transactions]) => {
      transactions.forEach(txn => {
        ...
        jtlContent += `  <httpSample t="${duration}" lt="${duration}" ts="${timestamp}" s="${success}" lb="${txn.name}" rc="${success ? '200' : '500'}" rm="${success ? 'OK' : (txn.error || 'Error')}" tn="User ${userId}" dt="text" by="0" ng="1" na="1">\n`;
        jtlContent += `    <responseData class="java.lang.String">${success ? 'Transaction completed successfully' : (txn.error || 'Transaction failed')}</responseData>\n`;
