/**
 * SUREPREP UI APPROVAL TEST - TestNG Compatible
 * 
 * This is the Java equivalent of tests/sureprep-ui-approval-test.js
 * Compatible with BOTH:
 * 1. TestNG execution (@Test annotation)
 * 2. Programmatic execution (main() method)
 * 
 ...
 * Run with main: java -cp ... SureprepUIApprovalTest
 */
...
public class SureprepUIApprovalTest {
    ...
    /**
     * TestNG test method - Entry point for TestNG execution
     * Parallel execution configured via @Test annotation parameters
     */
    @Test(threadPoolSize = 5, invocationCount = 1, timeOut = 600000)
    public void testSalesforceWorkflow() throws Exception {
        System.out.println("🚀 Sureprep UI Approval Test - TestNG Execution");
        System.out.println("📊 Test Parameters from @Test annotation:");
        System.out.println("   Thread Pool Size: 5");
        System.out.println("   Invocation Count: 1");
        
        // For TestNG execution, use thread ID as user ID
        int userId = (int) Thread.currentThread().getId();
        int iteration = 1;
        
        // Let exceptions propagate to TestNG - no suppression!
        runSalesforceWorkflow(userId, iteration);
    }
